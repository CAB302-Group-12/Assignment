 ...
 *
 */
public class HistoryInfoUI extends JFrame {
    ...
    private JTextField assetcategory;

    private JTextField assetquantity;

    private JTextField tradetype;
    ...
    /**
     * Constructor sets up history interface, adds listeners and displays.
     *
     * @param data The underlying data/model class the UI needs.
     */
    public HistoryInfoUI(HistoryInfoData data) {
        this.data = data;
        initUI();
        checkListSize();
    ...
    private JPanel makeAddressFieldsPanel() {
        ...
        hGroup.addGroup(layout.createParallelGroup().addComponent(assetnameLabel)
                .addComponent(assetcategoryLabel).addComponent(assetquantityLabel).addComponent(
                        tradetypeLabel).addComponent(organizationLabel));
        hGroup.addGroup(layout.createParallelGroup().addComponent(assetname)
                .addComponent(assetcategory).addComponent(assetquantity).addComponent(tradetype).addComponent(organization));
    ...
    private void setFieldsEditable(boolean editable) {
        ...
        assetcategory.setEditable(editable);
        assetquantity.setEditable(editable);
        tradetype.setEditable(editable);
     ...
     * @param history the Person to display.
     */
    private void display(History history) {
        if (history != null) {
            assetname.setText(history.getAssetname());
            assetcategory.setText(history.getAssetcategory());
            assetquantity.setText(history.getAssetquantity());
            tradetype.setText(history.getTradetype());
            organization.setText(history.getOrganization());
    ...
    private class ButtonListener implements ActionListener {
        ...
        private void savePressed() {
            if (assetname.getText() != null && !assetname.getText().equals("")) {
                History h = new History(String.valueOf(System.currentTimeMillis()), assetname.getText(), assetcategory.getText(), assetquantity
                        .getText(), tradetype.getText(), organization.getText());
                data.add(h);
            }
            setFieldsEditable(false);
            saveButton.setEnabled(false);
